...
- **Information-bar threshold is calibrated + fixed, not adaptive-EWMA** (Stage-1 fix). The AFML adaptive TIB/TRB threshold `E₀[T]·f(P)` **runs away and collapses** over long spans (carry-forward on equal-price ticks → small bars → `ema_T` ratchets to the floor → ~1 tick/bar; observed 17.6M bars from 49.5M EURUSD ticks). The tournament instead **calibrates a fixed threshold** (`calibrate_information_bar_threshold`, bisection on a bounded contiguous sample) so each candidate lands at the regime target bar count — a *fair* equal-count JB comparison. With this, EURUSD TIB beats time bars 16× on JB (1,625 vs 26,725); BTCUSD time wins honestly. The adaptive EWMA path stays available (`fixed_threshold=0.0`).
...
- **Per-stage observability** (`afml research diagnose`, `src/afml/research/diagnostics.py`). The CEO must be able to see *where* the pipeline succeeds or degrades, not just a binary "0 certified". Every trial carries a `TrialDiagnostics` (events → label balance → holding coherence vs regime V → MDA survivors → Brier vs naive → per-fold OOS Sharpe spread), persisted in the run artifact. `diagnose` renders the **attrition funnel** (where configs are lost across the 6 stages), the **validity heatmap** (█/· grid — plateau vs scattered spikes), per-stage **distributions**, and **feature-survival frequency** ("what carries signal"). Text/markdown now; the React control-plane dashboard is the follow-up surface over the same fields.
